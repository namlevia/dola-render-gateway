(function () {
    ...
    async function refreshAccountState(preserveStatus) {
        try {
            ...
            if (!preserveStatus) {
                setStatus(usable + ' tài khoản sẵn sàng · Tối đa ' + maxTabs + ' tab (2 tab/nick)', usable ? 'ready' : 'warning');
            }
    ...
    async function openRequestedAccounts() {
        ...
        if (!Number.isInteger(count) || count < 1) {
            setStatus('Vui lòng nhập số nguyên lớn hơn 0.', 'error');
            input.focus();
            return;
        }
        ...
        input.disabled = true;
        setStatus('Đang mở các tab độc lập…', 'working');
        ...
        try {
            ...
            if (!response.success) {
                const suffix = totalTabsOpened ? ' (' + totalTabsOpened + ' tab đã mở)' : '';
                setStatus((response.error || 'Không thể mở tab tài khoản.') + suffix, 'error');
                return;
            }

            setStatus('Đã mở thành công ' + openedCount + ' tài khoản (' + totalTabsOpened + ' tab).', 'success');
        } catch (error) {
            setStatus(error.message, 'error');
        } finally {
